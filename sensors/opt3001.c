...
double opt3001_get_data(I2C_Handle *i2c) {
	...
	if (opt3001_get_status(i2c) & OPT3001_DATA_READY) {
		...
		if (I2C_transfer(*i2c, &i2cMessage)) {
		    ...
		    uint16_t registry = (rxBuffer[0] << 8) | rxBuffer[1];

	        // JTKJ: Here the conversion from register value to lux
		    uint16_t E_BITMASK = 0b1111000000000000;
		    uint16_t R_BITMASK = 0b0000111111111111;
		    uint16_t E = (registry & E_BITMASK) >> 12;
		    uint16_t R = registry & R_BITMASK;
